...
import semseg_util.lib.psa.functional as PF
import models.resnet as rs
import models.sat_resnet as srs
...
class PSANet(nn.Module):
    def __init__(self, model_type='srs', num_bands=3, layers=50, dropout=0.1, classes=2, zoom_factor=8, use_psa=True, psa_type=2,
                 compact=False, shrink_factor=2, mask_h=59, mask_w=59, normalization_factor=1.0, psa_softmax=True,
        ...
        if model_type == 'rs':
            if layers == 50:
                ...
                resnet = rs.resnet152()
            ## Modify model. We don't care about the fc layer here since we don't use it
            del resnet.conv1
            resnet.conv1 = nn.Conv2d(num_bands, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
        elif model_type == 'srs':
            if layers == 50:
                resnet = srs.resnet50()
            elif layers == 101:
                resnet = srs.resnet101()
            else:
                resnet = srs.resnet152()
            ## Modify model. We don't care about the fc layer here since we don't use it
            del resnet.conv1
            resnet.conv1 = nn.Conv2d(num_bands, 64, kernel_size=3, stride=1, padding=1, bias=False)
        
        # self.layer0 = nn.Sequential(resnet.conv1, resnet.bn1, resnet.relu, resnet.conv2, resnet.bn2, resnet.relu, resnet.conv3, resnet.bn3, resnet.relu, resnet.maxpool)
        #self.layer0 = nn.Sequential(resnet.conv1, resnet.bn1, resnet.relu, resnet.maxpool)
        self.layer0 = nn.Sequential(resnet.conv1, resnet.bn1, nn.ReLU(inplace=True))
        self.layer1, self.layer2, self.layer3, self.layer4 = resnet.layer1, resnet.layer2, resnet.layer3, resnet.layer4
        
        ## Removing this for srs. messes with the sat_resnet shapes
        if model_type == 'rs':
    ...
    def forward(self, x, y=None):
        x_size = x.size()
        assert (x_size[2] - 1) % 8 == 0 and (x_size[3] - 1) % 8 == 0  ## causing issues with 32 size input
